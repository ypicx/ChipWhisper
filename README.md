...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Modules-50+-orange" alt="Modules">
...
| **Keil5 工程生成** | 生成完整的 `.uvprojx` + HAL 初始化 + 模块驱动（可选 Makefile + GCC 编译验证） |
...
### 板卡 (6 款)
- CT117E (蓝桥杯官方开发板 G431)
- Blue Pill (STM32F103C8T6 最小系统板)
- Nucleo-F103RB
- STM32F103C8 / F103RB 最小系统板
- STM32F103RB 实训板
...
graph TD
    ...
    D -->|无效| Z[END]
    E -->|可行| F[draft_app_logic<br/>生成业务逻辑IR]
    E -->|不可行| Z
    F --> G{review<br/>人机确认闸门}
    G -->|确认| H[scaffold<br/>生成工程文件+可选Makefile]
    G -->|打回| C
    H --> I[import_drivers<br/>导入 HAL 驱动]
    I --> J[build<br/>编译工程 Keil/GCC]
...
- [ARM GNU Toolchain](https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/downloads) — 可选，免费 GCC 编译验证（无需 Keil 也能验证代码正确性）
...
>   "arm_gcc_path": "D:\\Arm GNU Toolchain arm-none-eabi\\14.3 rel1\\bin\\arm-none-eabi-gcc.exe"
> }
> ```
...
# 3. 生成工程 + Makefile（启用 GCC 编译验证）
python -m stm32_agent scaffold request.json ./output --makefile

# 4. 导入 HAL 驱动
python -m stm32_agent import-cubef1-drivers ./output

# 5. 编译工程（Keil 或 GCC 自动检测）
python -m stm32_agent build-keil ./output
python -m stm32_agent build-project ./output          # 自动检测 Keil/GCC
python -m stm32_agent build-project ./output --gcc     # 强制使用 GCC
```
...
| **配置** | 管理 Keil/CubeMX/Renode/ARM GCC 路径 + LLM 模型 Profile + 连接测试 |
...
│   ├── keil_generator_makefile.py # Makefile + 链接脚本生成（GCC 编译用）
│   ├── gcc_builder.py     # ARM GCC 编译后端
│   ├── builder.py         # 构建抽象层（Keil / GCC 自动切换）
│   ├── app_logic_ir.py    # 业务逻辑中间表示
│   ├── app_logic_drafter.py # LLM 业务代码生成
│   ├── cli.py             # CLI 命令入口
│   └── ...
├── packs/                 # 扩展包
│   ├── modules/           # 50+ 模块定义和驱动模板
...
| `plan` | 规划引脚和资源分配 |
| `scaffold` | 生成 Keil5 工程骨架（`--makefile` 同时生成 Makefile） |
| `build-keil` | 调用 Keil 编译工程 |
| `build-project` | 自动检测 Keil/GCC 并编译（`--gcc` 强制 GCC） |
